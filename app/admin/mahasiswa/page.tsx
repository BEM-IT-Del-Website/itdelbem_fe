...
import { UserPlus, Shield, Crown, Award, CheckCircle, ChevronDown, ChevronRight, Users, Building, Trophy } from "lucide-react";
  ...
  const assignRole = async (studentId: number, role: string, category: string, organizationId?: number, organizationShortname?: string, positionTitle?: string) => {
    ...
    try {
      ...
      if (result.status === "success") {
        ...
        alert(`Role berhasil diberikan kepada mahasiswa!`);
      }
    } catch (err) {
                                ...
                                <button
                                  ...
                                  className={`w-full flex items-center justify-start p-3 text-left transition-all duration-200 hover:bg-gray-50 border-b border-gray-100 last:border-b-0 ${
                                    student.current_role === role.value && student.current_category === category.id && !student.current_organization_id
                                      ? 'bg-blue-50'
                                      : ''
                                  } ${assigningRole === student.id ? 'opacity-50 cursor-not-allowed' : ''}`}
                                >
