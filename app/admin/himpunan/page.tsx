...
const HimpunanPage: React.FC = () => {
  const [data, setData] = useState<Himpunan[]>([]);
  ...
  const fields = [
    { key: "name", label: "Nama Himpunan", type: "string" },
  ...
  const hasActiveFilters = searchName.trim() || searchProdi.trim() || searchStatus.trim();

  // Handle add new himpunan
  const handleAdd = () => {
    console.log("Add new himpunan");
    // TODO: buka modal form tambah
  };

  const handleEdit = (item: Himpunan) => {
    console.log("Edit item:", item);
    // TODO: buka modal form edit
  };

  const handleDelete = async (item: Himpunan) => {
    const token = sessionStorage.getItem("token");
    
    try {
      const res = await fetch(`http://localhost:8080/api/admin/himpunan/${item.id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const LoadingState = () => (
    <div className="min-h-96 flex items-center justify-center bg-white rounded-xl shadow-sm border border-gray-100">
      <div className="text-center space-y-4">
        ...
        <p className="text-gray-600 font-medium">Memuat data himpunan...</p>
    ...
    <div className="space-y-6 p-6 bg-gradient-to-br from-gray-50 to-white min-h-screen">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 mb-2">Data Himpunan Mahasiswa</h1>
          </div>
